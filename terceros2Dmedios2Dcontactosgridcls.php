...
$terceros2Dmedios2Dcontactos_grid = NULL; // Initialize page object first

class cterceros2Dmedios2Dcontactos_grid extends cterceros2Dmedios2Dcontactos {
	...
	// Page object name
	var $PageObjName = 'terceros2Dmedios2Dcontactos_grid';

	// Grid form hidden field names
	var $FormName = 'fterceros2Dmedios2Dcontactosgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["terceros2Dmedios2Dcontactos"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_idTercero") && $objForm->HasValue("o_idTercero") && $this->idTercero->CurrentValue <> $this->idTercero->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_denominacion") && $objForm->HasValue("o_denominacion") && $this->denominacion->CurrentValue <> $this->denominacion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idTipoContacto") && $objForm->HasValue("o_idTipoContacto") && $this->idTipoContacto->CurrentValue <> $this->idTipoContacto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_contacto") && $objForm->HasValue("o_contacto") && $this->contacto->CurrentValue <> $this->contacto->OldValue)
			return FALSE;
		return TRUE;
	}
