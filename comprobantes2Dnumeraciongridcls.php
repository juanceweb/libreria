...
$comprobantes2Dnumeracion_grid = NULL; // Initialize page object first

class ccomprobantes2Dnumeracion_grid extends ccomprobantes2Dnumeracion {
	...
	// Page object name
	var $PageObjName = 'comprobantes2Dnumeracion_grid';

	// Grid form hidden field names
	var $FormName = 'fcomprobantes2Dnumeraciongrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["comprobantes2Dnumeracion"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_puntoVenta") && $objForm->HasValue("o_puntoVenta") && $this->puntoVenta->CurrentValue <> $this->puntoVenta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ultimoNumero") && $objForm->HasValue("o_ultimoNumero") && $this->ultimoNumero->CurrentValue <> $this->ultimoNumero->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ultimoNumeroContable") && $objForm->HasValue("o_ultimoNumeroContable") && $this->ultimoNumeroContable->CurrentValue <> $this->ultimoNumeroContable->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		$this->puntoVenta->CurrentValue = 0;
		$this->puntoVenta->OldValue = $this->puntoVenta->CurrentValue;
		$this->ultimoNumero->CurrentValue = 0;
		$this->ultimoNumero->OldValue = $this->ultimoNumero->CurrentValue;
		$this->ultimoNumeroContable->CurrentValue = 0;
		$this->ultimoNumeroContable->OldValue = $this->ultimoNumeroContable->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->puntoVenta->setOldValue($objForm->GetValue("o_puntoVenta"));
		if (!$this->ultimoNumero->FldIsDetailKey) {
			$this->ultimoNumero->setFormValue($objForm->GetValue("x_ultimoNumero"));
		}
		$this->ultimoNumero->setOldValue($objForm->GetValue("o_ultimoNumero"));
		...
		$this->ultimoNumeroContable->setOldValue($objForm->GetValue("o_ultimoNumeroContable"));
	...
	function AddRow($rsold = NULL) {
		...
		// puntoVenta
		$this->puntoVenta->SetDbValueDef($rsnew, $this->puntoVenta->CurrentValue, 0, strval($this->puntoVenta->CurrentValue) == "");

		// ultimoNumero
		$this->ultimoNumero->SetDbValueDef($rsnew, $this->ultimoNumero->CurrentValue, 0, strval($this->ultimoNumero->CurrentValue) == "");

		// ultimoNumeroContable
		$this->ultimoNumeroContable->SetDbValueDef($rsnew, $this->ultimoNumeroContable->CurrentValue, 0, strval($this->ultimoNumeroContable->CurrentValue) == "");

		// idComprobante
		if ($this->idComprobante->getSessionValue() <> "") {
			$rsnew['idComprobante'] = $this->idComprobante->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "comprobantes") {
			...
			if ($GLOBALS["comprobantes"]->EventCancelled) $this->EventCancelled = TRUE;
		}
