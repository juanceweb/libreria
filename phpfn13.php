...
function &DbHelper($dbid = "") {
	if ($dbid == "walger1" || $dbid === 1) // walger1
		$dbclass = "cwalger1_db";
	elseif ($dbid == "ecommerce" || $dbid === 2) // ecommerce
		$dbclass = "cecommerce_db";
	else // DB
		$dbclass = "cvalle_db";
...
function CurrentProjectID() {
	...
	return "{7FFE1683-B3E8-4940-BA6E-6837E8BA6642}";
}
...
class cAdvancedSecurity {
	...
	function UserID_Loading() {
			...
			if(!isset($_SESSION["modo"])){
				$sql = "SELECT modo FROM `usuarios` where idUsuario='".$this->CurrentUserID."'"; 
				$rs = ew_LoadRecordset($sql);
				$rows = $rs->GetRows();
				$_SESSION["modo"] = $rows[0]["modo"];
			}
			$sql = "SELECT id,impresion FROM `comprobantes`"; 
			$rs = ew_LoadRecordset($sql);
			$rows = $rs->GetRows();
			for($i=0;$i<count($rows);$i++){
				$_SESSION["comprobantes"][$rows[$i]["id"]] = $rows[$i];	
			}
	}   
