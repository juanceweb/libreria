...
function Menu_Rendering(&$Menu) {
	if(isset($_SESSION["modo"])){
		if($_SESSION["modo"] != 2){
			?>
				<div style="position:fixed; top: 0; right: 10px; background-color:gray; padding: 10px; z-index:99999; box-shadow: 2px 2px 2px #ccc;">
					<h3 style="color: <?php echo $_SESSION["modo"] == 0?'black':'white'?>">
	...
	<script>
		...
		$(document).unbind('keypress').bind('keypress', function (e) {
			if(e.shiftKey && e.keyCode == 32){
				...
				if((e.keyCode == 48 || e.keyCode == 49 || e.keyCode == 50) && listening){
					listening = false
					console.log(window.location)
					window.location = 'articuloslist.php?modo=' + parseInt(e.keyCode-48);
				}else{
					listening = false
				}
			}
	...
	<?php
	$sql = "SELECT * FROM `accesos-directos`";
	...
	if ($rows && IsLoggedIn()) {
		if (count($rows) > 0) {
			...
			foreach ($rows as $key => $value) {
				?>
					<li style="list-style: none;padding-left: 20px;"><a class="" href="<?php echo $value["link"] ?>"><?php echo $value["texto"] ?></a></li>
				<?php
			}
		}		
	}	
