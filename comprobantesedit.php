...
$comprobantes_edit = NULL; // Initialize page object first

class ccomprobantes_edit extends ccomprobantes {
	...
	// Page object name
	var $PageObjName = 'comprobantes_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("comprobanteslist.php"); // Invalid key, return to list
		}
	...
	function RenderRow() {
		...
		$this->limitarModo->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->preimpreso);
	...
	function ValidateForm() {
		...
		if (in_array("comprobantes2Dnumeracion", $DetailTblVar) && $GLOBALS["comprobantes2Dnumeracion"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// discriminaIVA
			$this->discriminaIVA->SetDbValueDef($rsnew, $this->discriminaIVA->CurrentValue, NULL, $this->discriminaIVA->ReadOnly);

			// seAutoriza
			$this->seAutoriza->SetDbValueDef($rsnew, $this->seAutoriza->CurrentValue, NULL, $this->seAutoriza->ReadOnly);

			// letra
			$this->letra->SetDbValueDef($rsnew, $this->letra->CurrentValue, NULL, $this->letra->ReadOnly);

			// activo
			$this->activo->SetDbValueDef($rsnew, $this->activo->CurrentValue, NULL, $this->activo->ReadOnly);

			// ventaStock
			$this->ventaStock->SetDbValueDef($rsnew, $this->ventaStock->CurrentValue, NULL, $this->ventaStock->ReadOnly);

			// ventaStockReservadoVenta
			$this->ventaStockReservadoVenta->SetDbValueDef($rsnew, $this->ventaStockReservadoVenta->CurrentValue, NULL, $this->ventaStockReservadoVenta->ReadOnly);

			// ventaStockReservadoCompra
			$this->ventaStockReservadoCompra->SetDbValueDef($rsnew, $this->ventaStockReservadoCompra->CurrentValue, NULL, $this->ventaStockReservadoCompra->ReadOnly);

			// compraStock
			$this->compraStock->SetDbValueDef($rsnew, $this->compraStock->CurrentValue, NULL, $this->compraStock->ReadOnly);

			// compraStockReservadoVenta
			$this->compraStockReservadoVenta->SetDbValueDef($rsnew, $this->compraStockReservadoVenta->CurrentValue, NULL, $this->compraStockReservadoVenta->ReadOnly);

			// compraStockReservadoCompra
			$this->compraStockReservadoCompra->SetDbValueDef($rsnew, $this->compraStockReservadoCompra->CurrentValue, NULL, $this->compraStockReservadoCompra->ReadOnly);

			// muestraPendientes
			$this->muestraPendientes->SetDbValueDef($rsnew, $this->muestraPendientes->CurrentValue, NULL, $this->muestraPendientes->ReadOnly);

			// comprobanteDefaultImportacion
			$this->comprobanteDefaultImportacion->SetDbValueDef($rsnew, $this->comprobanteDefaultImportacion->CurrentValue, NULL, $this->comprobanteDefaultImportacion->ReadOnly);

			// preimpreso
			if ($this->preimpreso->Visible && !$this->preimpreso->ReadOnly && !$this->preimpreso->Upload->KeepFile) {
				$this->preimpreso->Upload->DbValue = $rsold['preimpreso']; // Get original value
			...
			// configuracionImpresion
			$this->configuracionImpresion->SetDbValueDef($rsnew, $this->configuracionImpresion->CurrentValue, NULL, $this->configuracionImpresion->ReadOnly);

			// configuracionImpresionCompra
			$this->configuracionImpresionCompra->SetDbValueDef($rsnew, $this->configuracionImpresionCompra->CurrentValue, NULL, $this->configuracionImpresionCompra->ReadOnly);

			// cantidadRegistros
			$this->cantidadRegistros->SetDbValueDef($rsnew, $this->cantidadRegistros->CurrentValue, NULL, $this->cantidadRegistros->ReadOnly);

			// limitarModo
			$this->limitarModo->SetDbValueDef($rsnew, $this->limitarModo->CurrentValue, NULL, $this->limitarModo->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("comprobantes2Dnumeracion", $DetailTblVar) && $GLOBALS["comprobantes2Dnumeracion"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["comprobantes2Dnumeracion_grid"]->GridUpdate();
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("comprobantes2Dnumeracion", $DetailTblVar)) {
				...
				if ($GLOBALS["comprobantes2Dnumeracion_grid"]->DetailEdit) {
					$GLOBALS["comprobantes2Dnumeracion_grid"]->CurrentMode = "edit";
					$GLOBALS["comprobantes2Dnumeracion_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($comprobantes_edit)) $comprobantes_edit = new ccomprobantes_edit();

// Page init
$comprobantes_edit->Page_Init();

// Page main
$comprobantes_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$comprobantes_edit->Page_Render();
?>
...
var CurrentForm = fcomprobantesedit = new ew_Form("fcomprobantesedit", "edit");

// Validate form
fcomprobantesedit.Validate = function() {
...
fcomprobantesedit.Form_CustomValidate = 
...
fcomprobantesedit.ValidateRequired = true;
<?php } else { ?>
fcomprobantesedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcomprobantesedit.Lists["x_discriminaIVA[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_discriminaIVA[]"].Options = <?php echo json_encode($comprobantes->discriminaIVA->Options()) ?>;
fcomprobantesedit.Lists["x_seAutoriza[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_seAutoriza[]"].Options = <?php echo json_encode($comprobantes->seAutoriza->Options()) ?>;
fcomprobantesedit.Lists["x_activo[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_activo[]"].Options = <?php echo json_encode($comprobantes->activo->Options()) ?>;
fcomprobantesedit.Lists["x_ventaStock"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_ventaStock"].Options = <?php echo json_encode($comprobantes->ventaStock->Options()) ?>;
fcomprobantesedit.Lists["x_ventaStockReservadoVenta"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_ventaStockReservadoVenta"].Options = <?php echo json_encode($comprobantes->ventaStockReservadoVenta->Options()) ?>;
fcomprobantesedit.Lists["x_ventaStockReservadoCompra"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_ventaStockReservadoCompra"].Options = <?php echo json_encode($comprobantes->ventaStockReservadoCompra->Options()) ?>;
fcomprobantesedit.Lists["x_compraStock"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_compraStock"].Options = <?php echo json_encode($comprobantes->compraStock->Options()) ?>;
fcomprobantesedit.Lists["x_compraStockReservadoVenta"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_compraStockReservadoVenta"].Options = <?php echo json_encode($comprobantes->compraStockReservadoVenta->Options()) ?>;
fcomprobantesedit.Lists["x_compraStockReservadoCompra"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_compraStockReservadoCompra"].Options = <?php echo json_encode($comprobantes->compraStockReservadoCompra->Options()) ?>;
fcomprobantesedit.Lists["x_muestraPendientes[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_muestraPendientes[]"].Options = <?php echo json_encode($comprobantes->muestraPendientes->Options()) ?>;
fcomprobantesedit.Lists["x_comprobanteDefaultImportacion"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"comprobantes"};
fcomprobantesedit.Lists["x_limitarModo"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcomprobantesedit.Lists["x_limitarModo"].Options = <?php echo json_encode($comprobantes->limitarModo->Options()) ?>;
...
<?php if (!$comprobantes_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $comprobantes_edit->ShowPageHeader(); ?>
<?php
$comprobantes_edit->ShowMessage();
?>
<form name="fcomprobantesedit" id="fcomprobantesedit" class="<?php echo $comprobantes_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($comprobantes_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $comprobantes_edit->Token ?>">
<?php } ?>
...
<?php if (@$_POST["fa_x_preimpreso"] == "0") { ?>
<input type="hidden" name="fa_x_preimpreso" id= "fa_x_preimpreso" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_preimpreso" id= "fa_x_preimpreso" value="1">
<?php } ?>
...
<input type="hidden" data-table="comprobantes" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($comprobantes->id->CurrentValue) ?>">
<?php
	if (in_array("comprobantes2Dnumeracion", explode(",", $comprobantes->getCurrentDetailTable())) && $comprobantes2Dnumeracion->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $comprobantes_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fcomprobantesedit.Init();
</script>
<?php
$comprobantes_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$comprobantes_edit->Page_Terminate();
?>
