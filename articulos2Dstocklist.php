...
$articulos2Dstock_list = NULL; // Initialize page object first

class carticulos2Dstock_list extends carticulos2Dstock {
	...
	// Page object name
	var $PageObjName = 'articulos2Dstock_list';

	// Grid form hidden field names
	var $FormName = 'farticulos2Dstocklist';
	...
	function __construct() {
		...
		$this->MultiDeleteUrl = "articulos2Dstockdelete.php";
		$this->MultiUpdateUrl = "articulos2Dstockupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption farticulos2Dstocklistsrch";
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->stockReservadoVenta, $bCtrl); // stockReservadoVenta
			$this->UpdateSort($this->stockReservadoCompra, $bCtrl); // stockReservadoCompra
			$this->UpdateSort($this->stockMinimo, $bCtrl); // stockMinimo
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->stock->setSort("");
				$this->stockReservadoVenta->setSort("");
				$this->stockReservadoCompra->setSort("");
				$this->stockMinimo->setSort("");
			}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"farticulos2Dstocklistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"farticulos2Dstocklistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.farticulos2Dstocklist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_articulos2Dstock\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_articulos2Dstock',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.farticulos2Dstocklist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($articulos2Dstock_list)) $articulos2Dstock_list = new carticulos2Dstock_list();

// Page init
$articulos2Dstock_list->Page_Init();

// Page main
$articulos2Dstock_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$articulos2Dstock_list->Page_Render();
?>
...
var CurrentForm = farticulos2Dstocklist = new ew_Form("farticulos2Dstocklist", "list");
farticulos2Dstocklist.FormKeyCountName = '<?php echo $articulos2Dstock_list->FormKeyCountName ?>';

// Form_CustomValidate event
farticulos2Dstocklist.Form_CustomValidate = 
...
farticulos2Dstocklist.ValidateRequired = true;
<?php } else { ?>
farticulos2Dstocklist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
farticulos2Dstocklist.Lists["x_idArticulo"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacionExterna","x_denominacionInterna","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"articulos"};
farticulos2Dstocklist.Lists["x_idUnidadMedida"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"unidades2Dmedida"};
...
<?php } ?>
<?php if ($articulos2Dstock_list->TotalRecs > 0 && $articulos2Dstock_list->ExportOptions->Visible()) { ?>
<?php $articulos2Dstock_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($articulos2Dstock->Export == "") { ?>
...
<?php } ?>
<?php if (($articulos2Dstock->Export == "") || (EW_EXPORT_MASTER_RECORD && $articulos2Dstock->Export == "print")) { ?>
<?php
if ($articulos2Dstock_list->DbMasterFilter <> "" && $articulos2Dstock->getCurrentMasterTable() == "articulos") {
	if ($articulos2Dstock_list->MasterRecordExists) {
?>
...
<?php } ?>
<?php
	$bSelectLimit = $articulos2Dstock_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($articulos2Dstock_list->TotalRecs <= 0)
			$articulos2Dstock_list->TotalRecs = $articulos2Dstock->SelectRecordCount();
	} else {
		if (!$articulos2Dstock_list->Recordset && ($articulos2Dstock_list->Recordset = $articulos2Dstock_list->LoadRecordset()))
			$articulos2Dstock_list->TotalRecs = $articulos2Dstock_list->Recordset->RecordCount();
	}
	$articulos2Dstock_list->StartRec = 1;
	if ($articulos2Dstock_list->DisplayRecs <= 0 || ($articulos2Dstock->Export <> "" && $articulos2Dstock->ExportAll)) // Display all records
		$articulos2Dstock_list->DisplayRecs = $articulos2Dstock_list->TotalRecs;
	if (!($articulos2Dstock->Export <> "" && $articulos2Dstock->ExportAll))
		$articulos2Dstock_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$articulos2Dstock_list->Recordset = $articulos2Dstock_list->LoadRecordset($articulos2Dstock_list->StartRec-1, $articulos2Dstock_list->DisplayRecs);

	// Set no record found message
	if ($articulos2Dstock->CurrentAction == "" && $articulos2Dstock_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$articulos2Dstock_list->setWarningMessage(ew_DeniedMsg());
		if ($articulos2Dstock_list->SearchWhere == "0=101")
			$articulos2Dstock_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$articulos2Dstock_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$articulos2Dstock_list->RenderOtherOptions();
?>
<?php $articulos2Dstock_list->ShowPageHeader(); ?>
<?php
$articulos2Dstock_list->ShowMessage();
?>
<?php if ($articulos2Dstock_list->TotalRecs > 0 || $articulos2Dstock->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid articulos2Dstock">
<?php if ($articulos2Dstock->Export == "") { ?>
<div class="panel-heading ewGridUpperPanel">
<?php if ($articulos2Dstock->CurrentAction <> "gridadd" && $articulos2Dstock->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($articulos2Dstock_list->Pager)) $articulos2Dstock_list->Pager = new cPrevNextPager($articulos2Dstock_list->StartRec, $articulos2Dstock_list->DisplayRecs, $articulos2Dstock_list->TotalRecs) ?>
<?php if ($articulos2Dstock_list->Pager->RecordCount > 0 && $articulos2Dstock_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($articulos2Dstock_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $articulos2Dstock_list->PageUrl() ?>start=<?php echo $articulos2Dstock_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($articulos2Dstock_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $articulos2Dstock_list->PageUrl() ?>start=<?php echo $articulos2Dstock_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $articulos2Dstock_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($articulos2Dstock_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $articulos2Dstock_list->PageUrl() ?>start=<?php echo $articulos2Dstock_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($articulos2Dstock_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $articulos2Dstock_list->PageUrl() ?>start=<?php echo $articulos2Dstock_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $articulos2Dstock_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $articulos2Dstock_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $articulos2Dstock_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $articulos2Dstock_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($articulos2Dstock_list->TotalRecs > 0) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="articulos2Dstock">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($articulos2Dstock_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($articulos2Dstock_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="40"<?php if ($articulos2Dstock_list->DisplayRecs == 40) { ?> selected<?php } ?>>40</option>
<option value="80"<?php if ($articulos2Dstock_list->DisplayRecs == 80) { ?> selected<?php } ?>>80</option>
<option value="160"<?php if ($articulos2Dstock_list->DisplayRecs == 160) { ?> selected<?php } ?>>160</option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($articulos2Dstock_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="farticulos2Dstocklist" id="farticulos2Dstocklist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($articulos2Dstock_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $articulos2Dstock_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="articulos2Dstock">
<?php if ($articulos2Dstock->getCurrentMasterTable() == "articulos" && $articulos2Dstock->CurrentAction <> "") { ?>
...
<?php if ($articulos2Dstock_list->TotalRecs > 0) { ?>
<table id="tbl_articulos2Dstocklist" class="table ewTable">
...
// Header row
$articulos2Dstock_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$articulos2Dstock_list->RenderListOptions();

// Render list options (header, left)
$articulos2Dstock_list->ListOptions->Render("header", "left");
?>
<?php if ($articulos2Dstock->idArticulo->Visible) { // idArticulo ?>
	...
	<?php } else { ?>
		<th data-name="idArticulo"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->idArticulo) ?>',2);"><div id="elh_articulos2Dstock_idArticulo" class="articulos2Dstock_idArticulo">
...
<?php if ($articulos2Dstock->idUnidadMedida->Visible) { // idUnidadMedida ?>
	...
	<?php } else { ?>
		<th data-name="idUnidadMedida"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->idUnidadMedida) ?>',2);"><div id="elh_articulos2Dstock_idUnidadMedida" class="articulos2Dstock_idUnidadMedida">
...
<?php if ($articulos2Dstock->indiceConversion->Visible) { // indiceConversion ?>
	...
	<?php } else { ?>
		<th data-name="indiceConversion"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->indiceConversion) ?>',2);"><div id="elh_articulos2Dstock_indiceConversion" class="articulos2Dstock_indiceConversion">
...
<?php if ($articulos2Dstock->stock->Visible) { // stock ?>
	...
	<?php } else { ?>
		<th data-name="stock"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->stock) ?>',2);"><div id="elh_articulos2Dstock_stock" class="articulos2Dstock_stock">
...
<?php if ($articulos2Dstock->stockReservadoVenta->Visible) { // stockReservadoVenta ?>
	...
	<?php } else { ?>
		<th data-name="stockReservadoVenta"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->stockReservadoVenta) ?>',2);"><div id="elh_articulos2Dstock_stockReservadoVenta" class="articulos2Dstock_stockReservadoVenta">
...
<?php if ($articulos2Dstock->stockReservadoCompra->Visible) { // stockReservadoCompra ?>
	...
	<?php } else { ?>
		<th data-name="stockReservadoCompra"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->stockReservadoCompra) ?>',2);"><div id="elh_articulos2Dstock_stockReservadoCompra" class="articulos2Dstock_stockReservadoCompra">
...
<?php if ($articulos2Dstock->stockMinimo->Visible) { // stockMinimo ?>
	...
	<?php } else { ?>
		<th data-name="stockMinimo"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $articulos2Dstock->SortUrl($articulos2Dstock->stockMinimo) ?>',2);"><div id="elh_articulos2Dstock_stockMinimo" class="articulos2Dstock_stockMinimo">
...
$articulos2Dstock_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($articulos2Dstock->ExportAll && $articulos2Dstock->Export <> "") {
	$articulos2Dstock_list->StopRec = $articulos2Dstock_list->TotalRecs;
} else {

	// Set the last record to display
	if ($articulos2Dstock_list->TotalRecs > $articulos2Dstock_list->StartRec + $articulos2Dstock_list->DisplayRecs - 1)
		$articulos2Dstock_list->StopRec = $articulos2Dstock_list->StartRec + $articulos2Dstock_list->DisplayRecs - 1;
	else
		$articulos2Dstock_list->StopRec = $articulos2Dstock_list->TotalRecs;
}
$articulos2Dstock_list->RecCnt = $articulos2Dstock_list->StartRec - 1;
if ($articulos2Dstock_list->Recordset && !$articulos2Dstock_list->Recordset->EOF) {
	$articulos2Dstock_list->Recordset->MoveFirst();
	$bSelectLimit = $articulos2Dstock_list->UseSelectLimit;
	if (!$bSelectLimit && $articulos2Dstock_list->StartRec > 1)
		$articulos2Dstock_list->Recordset->Move($articulos2Dstock_list->StartRec - 1);
} elseif (!$articulos2Dstock->AllowAddDeleteRow && $articulos2Dstock_list->StopRec == 0) {
	$articulos2Dstock_list->StopRec = $articulos2Dstock->GridAddRowCount;
}

// Initialize aggregate
$articulos2Dstock->RowType = EW_ROWTYPE_AGGREGATEINIT;
$articulos2Dstock->ResetAttrs();
$articulos2Dstock_list->RenderRow();
while ($articulos2Dstock_list->RecCnt < $articulos2Dstock_list->StopRec) {
	$articulos2Dstock_list->RecCnt++;
	if (intval($articulos2Dstock_list->RecCnt) >= intval($articulos2Dstock_list->StartRec)) {
		$articulos2Dstock_list->RowCnt++;

		// Set up key count
		$articulos2Dstock_list->KeyCount = $articulos2Dstock_list->RowIndex;
		...
		if ($articulos2Dstock->CurrentAction == "gridadd") {
			...
			$articulos2Dstock_list->LoadRowValues($articulos2Dstock_list->Recordset); // Load row values
		}
		$articulos2Dstock->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$articulos2Dstock->RowAttrs = array_merge($articulos2Dstock->RowAttrs, array('data-rowindex'=>$articulos2Dstock_list->RowCnt, 'id'=>'r' . $articulos2Dstock_list->RowCnt . '_articulos2Dstock', 'data-rowtype'=>$articulos2Dstock->RowType));
...
$articulos2Dstock_list->ListOptions->Render("body", "left", $articulos2Dstock_list->RowCnt);
?>
	<?php if ($articulos2Dstock->idArticulo->Visible) { // idArticulo ?>
		<td data-name="idArticulo"<?php echo $articulos2Dstock->idArticulo->CellAttributes() ?>>
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_idArticulo" class="articulos2Dstock_idArticulo">
<span<?php echo $articulos2Dstock->idArticulo->ViewAttributes() ?>>
<?php echo $articulos2Dstock->idArticulo->ListViewValue() ?></span>
</span>
<a id="<?php echo $articulos2Dstock_list->PageObjName . "_row_" . $articulos2Dstock_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($articulos2Dstock->idUnidadMedida->Visible) { // idUnidadMedida ?>
		<td data-name="idUnidadMedida"<?php echo $articulos2Dstock->idUnidadMedida->CellAttributes() ?>>
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_idUnidadMedida" class="articulos2Dstock_idUnidadMedida">
...
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_indiceConversion" class="articulos2Dstock_indiceConversion">
...
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_stock" class="articulos2Dstock_stock">
...
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_stockReservadoVenta" class="articulos2Dstock_stockReservadoVenta">
...
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_stockReservadoCompra" class="articulos2Dstock_stockReservadoCompra">
...
<span id="el<?php echo $articulos2Dstock_list->RowCnt ?>_articulos2Dstock_stockMinimo" class="articulos2Dstock_stockMinimo">
...
$articulos2Dstock_list->ListOptions->Render("body", "right", $articulos2Dstock_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($articulos2Dstock->CurrentAction <> "gridadd")
		$articulos2Dstock_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($articulos2Dstock->CurrentAction == "") { ?>
...
// Close recordset
if ($articulos2Dstock_list->Recordset)
	$articulos2Dstock_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($articulos2Dstock_list->TotalRecs == 0 && $articulos2Dstock->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($articulos2Dstock_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($articulos2Dstock->Export == "") { ?>
<script type="text/javascript">
farticulos2Dstocklist.Init();
</script>
<?php } ?>
<?php
$articulos2Dstock_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($articulos2Dstock->Export == "") { ?>
...
<?php
$articulos2Dstock_list->Page_Terminate();
?>
