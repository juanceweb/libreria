...
$categorias2Dterceros2Ddescuentos = NULL;

//
// Table class for categorias-terceros-descuentos
//
class ccategorias2Dterceros2Ddescuentos extends cTable {
	var $id;
	var $idCategoria;
	var $idTercero;
	var $descuento;
	...
	function __construct() {
		...
		$this->TableVar = 'categorias2Dterceros2Ddescuentos';
		$this->TableName = 'categorias-terceros-descuentos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`categorias-terceros-descuentos`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('categorias2Dterceros2Ddescuentos', 'categorias-terceros-descuentos', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = FALSE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// idCategoria
		$this->idCategoria = new cField('categorias2Dterceros2Ddescuentos', 'categorias-terceros-descuentos', 'x_idCategoria', 'idCategoria', '`idCategoria`', '`idCategoria`', 3, -1, FALSE, '`idCategoria`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// idTercero
		$this->idTercero = new cField('categorias2Dterceros2Ddescuentos', 'categorias-terceros-descuentos', 'x_idTercero', 'idTercero', '`idTercero`', '`idTercero`', 3, -1, FALSE, '`idTercero`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// descuento
		$this->descuento = new cField('categorias2Dterceros2Ddescuentos', 'categorias-terceros-descuentos', 'x_descuento', 'descuento', '`descuento`', '`descuento`', 4, -1, FALSE, '`descuento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "categorias2Darticulos") {
			if ($this->idCategoria->getSessionValue() <> "")
				$sMasterFilter .= "`id`=" . ew_QuotedValue($this->idCategoria->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "categorias2Darticulos") {
			if ($this->idCategoria->getSessionValue() <> "")
				$sDetailFilter .= "`idCategoria`=" . ew_QuotedValue($this->idCategoria->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "terceros") {
	...
	// Master filter
	function SqlMasterFilter_categorias2Darticulos() {
		return "`id`=@id@";
	}

	// Detail filter
	function SqlDetailFilter_categorias2Darticulos() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`categorias-terceros-descuentos`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "categorias2Dterceros2Ddescuentoslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("categorias2Dterceros2Ddescuentosview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("categorias2Dterceros2Ddescuentosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "categorias2Dterceros2Ddescuentosadd.php?" . $this->UrlParm($parm);
		else
			$url = "categorias2Dterceros2Ddescuentosadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("categorias2Dterceros2Ddescuentosedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("categorias2Dterceros2Ddescuentosadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("categorias2Dterceros2Ddescuentosdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "categorias2Darticulos" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->idCategoria->CurrentValue);
		}
   ...
   // Common render codes
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		clonarDescuentos($rsnew["idTercero"], 'all' , 'categoría');
	...
	function Row_Deleted(&$rs) {
		include_once('./funciones.php');
		clonarDescuentos($rs["idTercero"], 'all' , 'categoría');
