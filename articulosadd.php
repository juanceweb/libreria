...
$articulos_add = NULL; // Initialize page object first

class carticulos_add extends carticulos {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "articulosview.php")
	...
	function LoadDefaultValues() {
		$this->denominacionExterna->CurrentValue = NULL;
		$this->denominacionExterna->OldValue = $this->denominacionExterna->CurrentValue;
		$this->denominacionInterna->CurrentValue = NULL;
		$this->denominacionInterna->OldValue = $this->denominacionInterna->CurrentValue;
		...
		$this->idSubcateogoria->CurrentValue = NULL;
		$this->idSubcateogoria->OldValue = $this->idSubcateogoria->CurrentValue;
		$this->idRubro->CurrentValue = NULL;
		$this->idRubro->OldValue = $this->idRubro->CurrentValue;
		$this->idMarca->CurrentValue = NULL;
		$this->idMarca->OldValue = $this->idMarca->CurrentValue;
		$this->codigoBarras->CurrentValue = NULL;
		$this->codigoBarras->OldValue = $this->codigoBarras->CurrentValue;
		...
		$this->calculoPrecio->CurrentValue = 1;
		$this->rentabilidad->CurrentValue = NULL;
		$this->rentabilidad->OldValue = $this->rentabilidad->CurrentValue;
		$this->precioVenta->CurrentValue = NULL;
		$this->precioVenta->OldValue = $this->precioVenta->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("articulos2Dstock", $DetailTblVar) && $GLOBALS["articulos2Dstock"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// denominacionExterna
		$this->denominacionExterna->SetDbValueDef($rsnew, $this->denominacionExterna->CurrentValue, NULL, FALSE);

		// denominacionInterna
		$this->denominacionInterna->SetDbValueDef($rsnew, $this->denominacionInterna->CurrentValue, NULL, FALSE);
		...
		// idSubcateogoria
		$this->idSubcateogoria->SetDbValueDef($rsnew, $this->idSubcateogoria->CurrentValue, NULL, FALSE);

		// idRubro
		$this->idRubro->SetDbValueDef($rsnew, $this->idRubro->CurrentValue, NULL, FALSE);

		// idMarca
		$this->idMarca->SetDbValueDef($rsnew, $this->idMarca->CurrentValue, NULL, FALSE);

		// codigoBarras
		$this->codigoBarras->SetDbValueDef($rsnew, $this->codigoBarras->CurrentValue, NULL, FALSE);
		...
		// calculoPrecio
		$this->calculoPrecio->SetDbValueDef($rsnew, $this->calculoPrecio->CurrentValue, NULL, FALSE);

		// rentabilidad
		$this->rentabilidad->SetDbValueDef($rsnew, $this->rentabilidad->CurrentValue, NULL, FALSE);

		// precioVenta
		$this->precioVenta->SetDbValueDef($rsnew, $this->precioVenta->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("articulos2Dproveedores", $DetailTblVar) && $GLOBALS["articulos2Dproveedores"]->DetailAdd) {
				$GLOBALS["articulos2Dproveedores"]->idArticulo->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["articulos2Dproveedores_grid"])) $GLOBALS["articulos2Dproveedores_grid"] = new carticulos2Dproveedores_grid(); // Get detail page object
				...
				if (!$AddRow)
					$GLOBALS["articulos2Dproveedores"]->idArticulo->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("articulos2Dterceros2Ddescuentos", $DetailTblVar) && $GLOBALS["articulos2Dterceros2Ddescuentos"]->DetailAdd) {
				$GLOBALS["articulos2Dterceros2Ddescuentos"]->idArticulo->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["articulos2Dterceros2Ddescuentos_grid"])) $GLOBALS["articulos2Dterceros2Ddescuentos_grid"] = new carticulos2Dterceros2Ddescuentos_grid(); // Get detail page object
				...
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["articulos2Dterceros2Ddescuentos"]->idArticulo->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("articulos2Dstock", $DetailTblVar) && $GLOBALS["articulos2Dstock"]->DetailAdd) {
				$GLOBALS["articulos2Dstock"]->idArticulo->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["articulos2Dstock_grid"])) $GLOBALS["articulos2Dstock_grid"] = new carticulos2Dstock_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "articulos-stock"); // Load user level of detail table
				$AddRow = $GLOBALS["articulos2Dstock_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["articulos2Dstock"]->idArticulo->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("articulos2Dstock", $DetailTblVar)) {
				...
				if ($GLOBALS["articulos2Dstock_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["articulos2Dstock_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["articulos2Dstock_grid"]->CurrentMode = "add";
					$GLOBALS["articulos2Dstock_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($articulos_add)) $articulos_add = new carticulos_add();

// Page init
$articulos_add->Page_Init();

// Page main
$articulos_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$articulos_add->Page_Render();
?>
...
var CurrentForm = farticulosadd = new ew_Form("farticulosadd", "add");

// Validate form
farticulosadd.Validate = function() {
...
farticulosadd.Form_CustomValidate = 
...
farticulosadd.ValidateRequired = true;
<?php } else { ?>
farticulosadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
farticulosadd.Lists["x_idAlicuotaIva"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_valor","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"alicuotas2Diva"};
farticulosadd.Lists["x_idCategoria"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"categorias2Darticulos"};
farticulosadd.Lists["x_idSubcateogoria"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"subcategorias2Darticulos"};
farticulosadd.Lists["x_idRubro"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"rubros"};
farticulosadd.Lists["x_idMarca"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"marcas"};
farticulosadd.Lists["x_calculoPrecio"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
farticulosadd.Lists["x_calculoPrecio"].Options = <?php echo json_encode($articulos->calculoPrecio->Options()) ?>;
...
<?php } ?>
<?php $articulos_add->ShowPageHeader(); ?>
<?php
$articulos_add->ShowMessage();
?>
<form name="farticulosadd" id="farticulosadd" class="<?php echo $articulos_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($articulos_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $articulos_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("farticulosadd", "x_detalle", 35, 4, <?php echo ($articulos->detalle->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	if (in_array("articulos2Dproveedores", explode(",", $articulos->getCurrentDetailTable())) && $articulos2Dproveedores->DetailAdd) {
?>
...
<?php } ?>
<?php
	if (in_array("articulos2Dterceros2Ddescuentos", explode(",", $articulos->getCurrentDetailTable())) && $articulos2Dterceros2Ddescuentos->DetailAdd) {
?>
<?php if ($articulos->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("articulos2Dstock", explode(",", $articulos->getCurrentDetailTable())) && $articulos2Dstock->DetailAdd) {
?>
<?php if ($articulos->getCurrentDetailTable() <> "") { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $articulos_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
...
$(document).ready(function(){
	...
	$("#x_idUnidadMedidaCompra").html('<option value="1">Unidades</option>');
	$("#x_idUnidadMedidaVenta").html('<option value="1">Unidades</option>');
	$("#x_precioVenta").attr('readonly', true).attr('disabled', true);
})
$("#x_calculoPrecio").change(function(){
	if($(this).val() == 1){
		$("#x_precioVenta").attr('readonly', true).attr('disabled', true).val('');
		$("#x_rentabilidad").attr('readonly', false).attr('disabled', false);
	}else{
		$("#x_precioVenta").attr('readonly', false).attr('disabled', false);
		$("#x_rentabilidad").attr('readonly', true).attr('disabled', true).val('');
	}
})
</script>
<?php include_once "footer.php" ?>
<?php
$articulos_add->Page_Terminate();
?>
