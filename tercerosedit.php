...
$terceros_edit = NULL; // Initialize page object first

class cterceros_edit extends cterceros {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("terceroslist.php"); // Invalid key, return to list
		}
	...
	function ValidateForm() {
		...
		if (in_array("terceros2Dmedios2Dcontactos", $DetailTblVar) && $GLOBALS["terceros2Dmedios2Dcontactos"]->DetailEdit) {
		...
		if (in_array("subcategoria2Dterceros2Ddescuentos", $DetailTblVar) && $GLOBALS["subcategoria2Dterceros2Ddescuentos"]->DetailEdit) {
		...
		if (in_array("categorias2Dterceros2Ddescuentos", $DetailTblVar) && $GLOBALS["categorias2Dterceros2Ddescuentos"]->DetailEdit) {
		...
		if (in_array("sucursales", $DetailTblVar) && $GLOBALS["sucursales"]->DetailEdit) {
		...
		if (in_array("descuentosasociados", $DetailTblVar) && $GLOBALS["descuentosasociados"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// idTipoTercero
			$this->idTipoTercero->SetDbValueDef($rsnew, $this->idTipoTercero->CurrentValue, NULL, $this->idTipoTercero->ReadOnly);
			...
			// domicilioFiscal
			$this->domicilioFiscal->SetDbValueDef($rsnew, $this->domicilioFiscal->CurrentValue, NULL, $this->domicilioFiscal->ReadOnly);

			// idPaisFiscal
			$this->idPaisFiscal->SetDbValueDef($rsnew, $this->idPaisFiscal->CurrentValue, NULL, $this->idPaisFiscal->ReadOnly);

			// idProvinciaFiscal
			$this->idProvinciaFiscal->SetDbValueDef($rsnew, $this->idProvinciaFiscal->CurrentValue, NULL, $this->idProvinciaFiscal->ReadOnly);

			// idPartidoFiscal
			$this->idPartidoFiscal->SetDbValueDef($rsnew, $this->idPartidoFiscal->CurrentValue, NULL, $this->idPartidoFiscal->ReadOnly);

			// idLocalidadFiscal
			$this->idLocalidadFiscal->SetDbValueDef($rsnew, $this->idLocalidadFiscal->CurrentValue, NULL, $this->idLocalidadFiscal->ReadOnly);

			// calleFiscal
			$this->calleFiscal->SetDbValueDef($rsnew, $this->calleFiscal->CurrentValue, NULL, $this->calleFiscal->ReadOnly);

			// direccionFiscal
			$this->direccionFiscal->SetDbValueDef($rsnew, $this->direccionFiscal->CurrentValue, NULL, $this->direccionFiscal->ReadOnly);

			// tipoDoc
			$this->tipoDoc->SetDbValueDef($rsnew, $this->tipoDoc->CurrentValue, NULL, $this->tipoDoc->ReadOnly);

			// documento
			$this->documento->SetDbValueDef($rsnew, $this->documento->CurrentValue, NULL, $this->documento->ReadOnly);

			// condicionIva
			$this->condicionIva->SetDbValueDef($rsnew, $this->condicionIva->CurrentValue, NULL, $this->condicionIva->ReadOnly);

			// observaciones
			$this->observaciones->SetDbValueDef($rsnew, $this->observaciones->CurrentValue, NULL, $this->observaciones->ReadOnly);

			// idTransporte
			$this->idTransporte->SetDbValueDef($rsnew, $this->idTransporte->CurrentValue, NULL, $this->idTransporte->ReadOnly);

			// idVendedor
			$this->idVendedor->SetDbValueDef($rsnew, $this->idVendedor->CurrentValue, NULL, $this->idVendedor->ReadOnly);

			// idCobrador
			$this->idCobrador->SetDbValueDef($rsnew, $this->idCobrador->CurrentValue, NULL, $this->idCobrador->ReadOnly);

			// comision
			$this->comision->SetDbValueDef($rsnew, $this->comision->CurrentValue, NULL, $this->comision->ReadOnly);

			// idListaPrecios
			$this->idListaPrecios->SetDbValueDef($rsnew, $this->idListaPrecios->CurrentValue, NULL, $this->idListaPrecios->ReadOnly);

			// dtoCliente
			$this->dtoCliente->SetDbValueDef($rsnew, $this->dtoCliente->CurrentValue, NULL, $this->dtoCliente->ReadOnly);
			...
			// limiteDescubierto
			$this->limiteDescubierto->SetDbValueDef($rsnew, $this->limiteDescubierto->CurrentValue, NULL, $this->limiteDescubierto->ReadOnly);

			// codigoPostal
			$this->codigoPostal->SetDbValueDef($rsnew, $this->codigoPostal->CurrentValue, NULL, $this->codigoPostal->ReadOnly);

			// codigoPostalFiscal
			$this->codigoPostalFiscal->SetDbValueDef($rsnew, $this->codigoPostalFiscal->CurrentValue, NULL, $this->codigoPostalFiscal->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("terceros2Dmedios2Dcontactos", $DetailTblVar) && $GLOBALS["terceros2Dmedios2Dcontactos"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["terceros2Dmedios2Dcontactos_grid"]->GridUpdate();
				...
				if ($EditRow) {
					if (in_array("subcategoria2Dterceros2Ddescuentos", $DetailTblVar) && $GLOBALS["subcategoria2Dterceros2Ddescuentos"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["subcategoria2Dterceros2Ddescuentos_grid"]->GridUpdate();
						$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
					}
				}
				if ($EditRow) {
					if (in_array("categorias2Dterceros2Ddescuentos", $DetailTblVar) && $GLOBALS["categorias2Dterceros2Ddescuentos"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["categorias2Dterceros2Ddescuentos_grid"]->GridUpdate();
						$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
					}
				}
				if ($EditRow) {
					if (in_array("sucursales", $DetailTblVar) && $GLOBALS["sucursales"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["sucursales_grid"]->GridUpdate();
						$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
					}
				}
				if ($EditRow) {
					if (in_array("descuentosasociados", $DetailTblVar) && $GLOBALS["descuentosasociados"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["descuentosasociados_grid"]->GridUpdate();
						$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("terceros2Dmedios2Dcontactos", $DetailTblVar)) {
				...
				if ($GLOBALS["terceros2Dmedios2Dcontactos_grid"]->DetailEdit) {
					$GLOBALS["terceros2Dmedios2Dcontactos_grid"]->CurrentMode = "edit";
					$GLOBALS["terceros2Dmedios2Dcontactos_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("subcategoria2Dterceros2Ddescuentos", $DetailTblVar)) {
				...
				if ($GLOBALS["subcategoria2Dterceros2Ddescuentos_grid"]->DetailEdit) {
					$GLOBALS["subcategoria2Dterceros2Ddescuentos_grid"]->CurrentMode = "edit";
					$GLOBALS["subcategoria2Dterceros2Ddescuentos_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("categorias2Dterceros2Ddescuentos", $DetailTblVar)) {
				...
				if ($GLOBALS["categorias2Dterceros2Ddescuentos_grid"]->DetailEdit) {
					$GLOBALS["categorias2Dterceros2Ddescuentos_grid"]->CurrentMode = "edit";
					$GLOBALS["categorias2Dterceros2Ddescuentos_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("sucursales", $DetailTblVar)) {
				...
				if ($GLOBALS["sucursales_grid"]->DetailEdit) {
					$GLOBALS["sucursales_grid"]->CurrentMode = "edit";
					$GLOBALS["sucursales_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("descuentosasociados", $DetailTblVar)) {
				...
				if ($GLOBALS["descuentosasociados_grid"]->DetailEdit) {
					$GLOBALS["descuentosasociados_grid"]->CurrentMode = "edit";
					$GLOBALS["descuentosasociados_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($terceros_edit)) $terceros_edit = new cterceros_edit();

// Page init
$terceros_edit->Page_Init();

// Page main
$terceros_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$terceros_edit->Page_Render();
?>
...
var CurrentForm = ftercerosedit = new ew_Form("ftercerosedit", "edit");

// Validate form
ftercerosedit.Validate = function() {
...
ftercerosedit.Form_CustomValidate = 
...
ftercerosedit.ValidateRequired = true;
<?php } else { ?>
ftercerosedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftercerosedit.Lists["x_idTipoTercero"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipos2Dterceros"};
ftercerosedit.Lists["x_idPais"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":["x_idProvincia"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"paises"};
ftercerosedit.Lists["x_idProvincia"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":["x_idPais"],"ChildFields":["x_idPartido"],"FilterFields":["x_idPais"],"Options":[],"Template":"","LinkTable":"provincias"};
ftercerosedit.Lists["x_idPartido"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":["x_idProvincia"],"ChildFields":["x_idLocalidad"],"FilterFields":["x_idProvincia"],"Options":[],"Template":"","LinkTable":"partidos"};
ftercerosedit.Lists["x_idLocalidad"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":["x_idPartido"],"ChildFields":[],"FilterFields":["x_idPartido"],"Options":[],"Template":"","LinkTable":"localidades"};
ftercerosedit.Lists["x_domicilioFiscal"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftercerosedit.Lists["x_domicilioFiscal"].Options = <?php echo json_encode($terceros->domicilioFiscal->Options()) ?>;
ftercerosedit.Lists["x_idPaisFiscal"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":["x_idProvinciaFiscal"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"paises"};
ftercerosedit.Lists["x_idProvinciaFiscal"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":["x_idPaisFiscal"],"ChildFields":["x_idPartidoFiscal"],"FilterFields":["x_idPais"],"Options":[],"Template":"","LinkTable":"provincias"};
ftercerosedit.Lists["x_idPartidoFiscal"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":["x_idProvinciaFiscal"],"ChildFields":["x_idLocalidadFiscal"],"FilterFields":["x_idProvincia"],"Options":[],"Template":"","LinkTable":"partidos"};
ftercerosedit.Lists["x_idLocalidadFiscal"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":["x_idPartidoFiscal"],"ChildFields":[],"FilterFields":["x_idPartido"],"Options":[],"Template":"","LinkTable":"localidades"};
ftercerosedit.Lists["x_tipoDoc"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipos2Ddocumentos"};
ftercerosedit.Lists["x_condicionIva"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"condiciones2Diva"};
ftercerosedit.Lists["x_idTransporte"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"terceros"};
ftercerosedit.Lists["x_idVendedor"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"terceros"};
ftercerosedit.Lists["x_idCobrador"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"terceros"};
ftercerosedit.Lists["x_idListaPrecios"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","x_descuento","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"lista2Dprecios"};
...
<?php } ?>
<?php $terceros_edit->ShowPageHeader(); ?>
<?php
$terceros_edit->ShowMessage();
?>
<form name="ftercerosedit" id="ftercerosedit" class="<?php echo $terceros_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($terceros_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $terceros_edit->Token ?>">
<?php } ?>
...
<input type="hidden" data-table="terceros" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($terceros->id->CurrentValue) ?>">
<?php
	if (in_array("terceros2Dmedios2Dcontactos", explode(",", $terceros->getCurrentDetailTable())) && $terceros2Dmedios2Dcontactos->DetailEdit) {
?>
...
<?php } ?>
<?php
	if (in_array("articulos2Dterceros2Ddescuentos", explode(",", $terceros->getCurrentDetailTable())) && $articulos2Dterceros2Ddescuentos->DetailEdit) {
?>
<?php if ($terceros->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("articulos2Dproveedores", explode(",", $terceros->getCurrentDetailTable())) && $articulos2Dproveedores->DetailEdit) {
?>
<?php if ($terceros->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("subcategoria2Dterceros2Ddescuentos", explode(",", $terceros->getCurrentDetailTable())) && $subcategoria2Dterceros2Ddescuentos->DetailEdit) {
?>
<?php if ($terceros->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("categorias2Dterceros2Ddescuentos", explode(",", $terceros->getCurrentDetailTable())) && $categorias2Dterceros2Ddescuentos->DetailEdit) {
?>
<?php if ($terceros->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("sucursales", explode(",", $terceros->getCurrentDetailTable())) && $sucursales->DetailEdit) {
?>
<?php if ($terceros->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("descuentosasociados", explode(",", $terceros->getCurrentDetailTable())) && $descuentosasociados->DetailEdit) {
?>
<?php if ($terceros->getCurrentDetailTable() <> "") { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $terceros_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
