...
$terceros2Dcontactos = NULL;

//
// Table class for terceros-contactos
//
class cterceros2Dcontactos extends cTable {
	var $id;
	var $idTercero;
	var $idPersona;
	var $principal;
	...
	function __construct() {
		...
		$this->TableVar = 'terceros2Dcontactos';
		$this->TableName = 'terceros-contactos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`terceros-contactos`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('terceros2Dcontactos', 'terceros-contactos', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = FALSE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// idTercero
		$this->idTercero = new cField('terceros2Dcontactos', 'terceros-contactos', 'x_idTercero', 'idTercero', '`idTercero`', '`idTercero`', 3, -1, FALSE, '`idTercero`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// idPersona
		$this->idPersona = new cField('terceros2Dcontactos', 'terceros-contactos', 'x_idPersona', 'idPersona', '`idPersona`', '`idPersona`', 3, -1, FALSE, '`idPersona`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->idPersona->Sortable = TRUE; // Allow sort
		$this->idPersona->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->idPersona->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->idPersona->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idPersona'] = &$this->idPersona;

		// principal
		$this->principal = new cField('terceros2Dcontactos', 'terceros-contactos', 'x_principal', 'principal', '`principal`', '`principal`', 3, -1, FALSE, '`principal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->principal->Sortable = TRUE; // Allow sort
		$this->principal->OptionCount = 1;
		$this->principal->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`terceros-contactos`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "terceros2Dcontactoslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("terceros2Dcontactosview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("terceros2Dcontactosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "terceros2Dcontactosadd.php?" . $this->UrlParm($parm);
		else
			$url = "terceros2Dcontactosadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("terceros2Dcontactosedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("terceros2Dcontactosadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("terceros2Dcontactosdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->principal->Exportable) $Doc->ExportCaption($this->principal);
				} else {
					if ($this->idTercero->Exportable) $Doc->ExportCaption($this->idTercero);
					if ($this->idPersona->Exportable) $Doc->ExportCaption($this->idPersona);
					if ($this->principal->Exportable) $Doc->ExportCaption($this->principal);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->principal->Exportable) $Doc->ExportField($this->principal);
					} else {
						if ($this->idTercero->Exportable) $Doc->ExportField($this->idTercero);
						if ($this->idPersona->Exportable) $Doc->ExportField($this->idPersona);
						if ($this->principal->Exportable) $Doc->ExportField($this->principal);
