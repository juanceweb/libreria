...
$articulospendientes_php = NULL; // Initialize page object first

class carticulospendientes_php {
	...
	// Table name
	var $TableName = 'articulospendientes.php';

	// Page object name
	var $PageObjName = 'articulospendientes_php';
	...
	function __construct() {
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'articulospendientes.php', TRUE);
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("custom", "articulospendientes_php", $url, "", "articulospendientes_php", TRUE);
	}
}
?>
<?php ew_Header(FALSE) ?>
...
// Create page object
if (!isset($articulospendientes_php)) $articulospendientes_php = new carticulospendientes_php();

// Page init
$articulospendientes_php->Page_Init();

// Page main
$articulospendientes_php->Page_Main();
...
<div class="container">
	...
	<script>
		...
		function ejecutar(){
			...
			var accion = "articulos-pendientes";
		    		...
		    		for (var i = 0; i< Object.keys(response["exito"]).length ; i++) {
		    			...
		    			if (response["exito"][i]["tercero"] != ultimotercero) {
		    				...
		    				html += '<tr style="font-weight: bold; background-color:#666; color:white">';
			 	   			...
			 	   			html +='<td style="font-weight: bold; background-color:#666; color:white">Artículo</td>';
		    			...
		    			html += '<td>'+response["exito"][i]["comprobante"] + " " + response["exito"][i]["numeroComprobante"]+'</td>';
		    			html += '<td>'+response["exito"][i]["denominacionArticulo"]+'</td>';
...
<?php
$articulospendientes_php->Page_Terminate();
?>
