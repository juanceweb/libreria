...
$comprobantes2Dbloqueados2Dcondiciones2Diva_grid = NULL; // Initialize page object first

class ccomprobantes2Dbloqueados2Dcondiciones2Diva_grid extends ccomprobantes2Dbloqueados2Dcondiciones2Diva {
	...
	// Page object name
	var $PageObjName = 'comprobantes2Dbloqueados2Dcondiciones2Diva_grid';

	// Grid form hidden field names
	var $FormName = 'fcomprobantes2Dbloqueados2Dcondiciones2Divagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["comprobantes2Dbloqueados2Dcondiciones2Diva"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_idCondicionIva") && $objForm->HasValue("o_idCondicionIva") && $this->idCondicionIva->CurrentValue <> $this->idCondicionIva->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idComprobanteBloqueado") && $objForm->HasValue("o_idComprobanteBloqueado") && $this->idComprobanteBloqueado->CurrentValue <> $this->idComprobanteBloqueado->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->idCondicionIva->setOldValue($objForm->GetValue("o_idCondicionIva"));
		...
		$this->idComprobanteBloqueado->setOldValue($objForm->GetValue("o_idComprobanteBloqueado"));
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// idComprobanteBloqueado
			$this->idComprobanteBloqueado->SetDbValueDef($rsnew, $this->idComprobanteBloqueado->CurrentValue, NULL, $this->idComprobanteBloqueado->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "condiciones2Diva") {
			$this->idCondicionIva->Visible = FALSE;
			if ($GLOBALS["condiciones2Diva"]->EventCancelled) $this->EventCancelled = TRUE;
		}
